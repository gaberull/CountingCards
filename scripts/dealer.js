/*
*   dealer.js
*   CountingCards   -   gabrielscott.io
*
*   Copyright (c) 2023 Gabe Scott
*
*/

const MAX_RELOAD = 1000;  
...
class dealer {
    ...
    constructor(numPlayers) {
        if(typeof(numPlayers) !== "number") {
            throw new Error('expected type number, got ' + typeof numPlayers);
        }
        this.#numPlayers = numPlayers;
        this.#otherPlayerHands = new Array();    // TODO maybe add numPlayers - 1 in ()
        this.#handArray = new Array();           // maybe add 1 in ()
        this.#dealerHand = new Hand();
    }
    hasBlackjack() {
        return this.#dealerHand.isBlackjack();
    }
    getMaxBet(playerBank)
    {
        return Math.min(MAX_RELOAD, playerBank.getBalance());
    }

    dealHands(shoe, playerBank, bet) {
        // check for overbet at slider. Set limit
        this.#dealerHand = new Hand();
        this.#handArray = [];
        this.#otherPlayerHands = [];
        playerBank.removeFunds(bet);

        let playerHand = new Hand(shoe.dealCard(), shoe.dealCard(), bet);
        this.#handArray.push(playerHand);
        ...
        // }
        //TODO: add animation for hole card checking if first card is 10 or Ace
...
int Dealer::action(Shoe* shoe, Bank* playerBank, char action, bool test)
{
    ...
    switch (action) {
        ...
        case 'P': {
            cout << "\n";
            cout << "Player Stands with   :    "<< playerHand.getHand() << " \n";
        ...
        case 'S': {
            ...
            else    // split the hand
            {
                ...
                // new split hands
                cout << "\n";
                cout << "Hand 1               :    " << playerHand.getHand() <<"  bet: $"<<playerHand.getBet()<< "\n";
                cout << "\n";
                cout << "Hand 2               :    " << newHand.getHand() <<"  bet: $"<<playerHand.getBet()<< "\n";
